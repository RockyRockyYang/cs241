...
void *mini_malloc(size_t size, const char *file, size_t line) {
  // your code here
    meta_data *p = malloc(sizeof(meta_data)+size);
    if(!p) return NULL;
    insert_meta_data(p, size, file, line);
    return(p+1);
}
...
void remove_meta_data(void *ptr) {
  /* check if ptr is in the list and delete it from list */
    ...
    while(move->next){
        if(move->next == md){
            meta_data *delete = move->next;
            move->next = delete->next;
            total_free += delete->size;
            free(delete);
            delete = NULL;
            return;
        }
